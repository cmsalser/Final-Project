...
export class DFAFactory extends Factory{
    ...
    override build(data: AstNode): void {
        ...
        //See github issue https://github.com/DmitrySoshnikov/regexp-tree/issues/252
        //Find start state by finding the state with no incoming transitions
        let n = this.nodes.find(n => {
            if (this.transitions.filter(t => t.to == n).length == 0) return n;
            else return; 
        });
        n!.setStart(true);
    }
